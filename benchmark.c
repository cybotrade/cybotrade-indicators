...
typedef int (*run_fun)(const ti_indicator_info *info, TI_REAL *options,
                       int goal);
...
typedef TA_RetCode (*TA_CdlFunction)(int startIdx, int endIdx,
                                     const double inOpen[],
                                     const double inHigh[],
                                     const double inLow[],
                                     const double inClose[], int *outBegIdx,
                                     int *outNBElement, int outInteger[]);

typedef struct cnd_cross_ref {
  tc_set pattern;
  TA_CdlFunction tacdl;
  int count; /* 0 to count any, -1 to only count negatives, +1 to only count
                positives */
} cnd_cross_ref;
...
void setup_talib() {
  ...
  TA_SetCandleSettings(TA_BodyDoji, TA_RangeType_HighLow, config.period,
                       config.body_none);
  TA_SetCandleSettings(TA_BodyShort, TA_RangeType_RealBody, config.period,
                       config.body_short);
  TA_SetCandleSettings(TA_BodyLong, TA_RangeType_RealBody, config.period,
                       config.body_long);

  TA_SetCandleSettings(TA_ShadowVeryShort, TA_RangeType_HighLow, config.period,
                       config.wick_none);
  TA_SetCandleSettings(TA_ShadowLong, TA_RangeType_RealBody, config.period,
                       config.wick_long);

  TA_SetCandleSettings(TA_Near, TA_RangeType_HighLow, config.period,
                       config.near);
}
  ...
  if (goal == GOAL_RUN) {
    ...
    for (i = 0; i < (int)info->nbInput; ++i) {
      ...
      if (input->type == TA_Input_Price) {
        r = TA_SetInputParamPricePtr(ph, (unsigned int)i, in[0], in[1], in[2],
                                     in[3], in[4], 0);
        CHECKR("set price");
...
int run_ti(const ti_indicator_info *info, TI_REAL *options, int goal) {
  ...
  if (goal == GOAL_RUN) {
    ...
    /* Run it. */
    const int ret = info->indicator(INSIZE, (TI_REAL const *const *)inputs,
                                    options, outputs);
    if (ret != TI_OKAY) {
      printf("   *ERROR* (%d)\n", ret);
      printf("options:\n");
...
int run_ti_ref(const ti_indicator_info *info, TI_REAL *options, int goal) {
  static TI_REAL *inputs[] = {0, 0, 0, 0, 0};
  static TI_REAL *outputs[] = {outref[0], outref[1], outref[2], outref[3],
                               outref[4]};
  ...
  if (goal == GOAL_RUN) {
    ...
    /* Run it. */
    const int ret = info->indicator_ref(INSIZE, (TI_REAL const *const *)inputs,
                                        options, outputs);
    if (ret != TI_OKAY) {
      printf("   *ERROR* (%d)\n", ret);
      printf("options:\n");
...
int run_ti_stream1(const ti_indicator_info *info, TI_REAL *options, int goal) {
  static TI_REAL *inputs[] = {0, 0, 0, 0, 0};
  static TI_REAL *outputs[] = {outstream1[0], outstream1[1], outstream1[2],
                               outstream1[3], outstream1[4]};
  ...
  if (goal == GOAL_RUN) {
    ...
    for (int i = 0; i < INSIZE; ++i) {
      ...
      const int ret =
          info->stream_run(stream, 1, (TI_REAL const *const *)ins, outs);
...
int run_ti_streamall(const ti_indicator_info *info, TI_REAL *options,
                     int goal) {
  static TI_REAL *inputs[] = {0, 0, 0, 0, 0};
  static TI_REAL *outputs[] = {outstreamall[0], outstreamall[1],
                               outstreamall[2], outstreamall[3],
                               outstreamall[4]};
  ...
  if (goal == GOAL_RUN) {
    ...
    const int ret = info->stream_run(stream, INSIZE,
                                     (TI_REAL const *const *)inputs, outputs);
    info->stream_free(stream);
...
// TODO These should have less constants, instead relying on some manipulation
// of period
// TODO It seems there are too many functions here. It's likely a few could be
// combined.
...
void check_outputs(const ti_indicator_info *info, const char *what,
                   TI_REAL compare[MAX_OUT_SERIES][INSIZE]) {
  ...
  if (ta) {
    if (strcmp(info->name, "stochrsi") == 0)
      output_adjust = stochrsi_output_adjust;

    if (strcmp(info->name, "bbands") == 0) {
      remap[0] = 2;
      remap[2] = 0;
    }
    if (strcmp(info->name, "dm") == 0) {
      remap[0] = 0;
      remap[1] = -1;
    }
    if (strcmp(info->name, "di") == 0) {
      remap[0] = 0;
      remap[1] = -1;
    }
  }
...
void bench_run(FILE *log, const ti_indicator_info *info, const void *run_info,
               run_fun fun, const char *name) {

  /* Options storage. */
  /* NB some talib functions take an extra scalar option, so we set the default
   * to 1. */
  ...
  if (strcmp(info->name, "alma") == 0) {
    options_setter = alma_option_setter;
  }
  if (strcmp(info->name, "adosc") == 0) {
    options_setter = fast_slow_option_setter;
  }
  if (strcmp(info->name, "apo") == 0) {
    options_setter = ppo_option_setter;
  }
  if (strcmp(info->name, "bbands") == 0) {
    options_setter = bbands_option_setter;
  }
  if (strcmp(info->name, "copp") == 0) {
    options_setter = copp_option_setter;
  }
  if (strcmp(info->name, "kc") == 0) {
    options_setter = kc_option_setter;
  }
  if (strcmp(info->name, "kst") == 0) {
    options_setter = kst_option_setter;
  }
  if (strcmp(info->name, "kvo") == 0) {
    options_setter = fast_slow_option_setter;
  }
  if (strcmp(info->name, "macd") == 0) {
    options_setter = macd_option_setter;
  }
  if (strcmp(info->name, "mama") == 0) {
    options_setter = mama_option_setter;
  }
  if (strcmp(info->name, "pfe") == 0) {
    options_setter = pfe_option_setter;
  }
  if (strcmp(info->name, "posc") == 0) {
    options_setter = posc_option_setter;
  }
  if (strcmp(info->name, "ppo") == 0) {
    options_setter = ppo_option_setter;
  }
  if (strcmp(info->name, "psar") == 0) {
    options_setter = psar_option_setter;
  }
  if (strcmp(info->name, "rvi") == 0) {
    options_setter = rvi_option_setter;
  }
  if (strcmp(info->name, "rmi") == 0) {
    options_setter = rmi_option_setter;
  }
  if (strcmp(info->name, "rmta") == 0) {
    options_setter = rmta_option_setter;
  }
  if (strcmp(info->name, "smi") == 0) {
    options_setter = smi_option_setter;
  }
  if (strcmp(info->name, "stoch") == 0) {
    options_setter = stoch_option_setter;
  }
  if (strcmp(info->name, "stochrsi") == 0) {
    options_setter = stochrsi_option_setter;
  }
  if (strcmp(info->name, "ultosc") == 0) {
    options_setter = ultosc_option_setter;
  }
  if (strcmp(info->name, "vidya") == 0) {
    options_setter = vidya_option_setter;
  }
  ...
  for (best = 0; best < bestof; ++best) {
    ...
    const int performance =
        elapsed == 0 ? 0 : (iterations * INSIZE) / elapsed / 1000;
  ...
  if (log)
    fprintf(log,
            ",\n    \"%s\" => array(\"elapsed\" => %d, \"performance\" => %d)",
            name, best_e, best_p);
}
...
void bench_run_candle_talib(FILE *log, const tc_candle_info *info,
                            const cnd_cross_ref *c) {
  ...
  for (best = 0; best < bestof; ++best) {
    ...
    for (i = 0; i < LOOPS; ++i) {
      for (period = PSTART; period <= PEND; ++period) {
        ...
        int count;
        int ret = c->tacdl(0, INSIZE - 1, in[0], in[1], in[2], in[3],
                           &ta_offset, &count, outtaint);
        if (ret) {
          return;
        }
        ++iterations;
...
int main(int argc, char **argv) {
  ...
  if (argc > 1 && !isdigit(argv[1][0]) && !candles_only) {
    ...
    if (!log) {
      printf(
          "Couldn't open results file for output. Make sure ./docs/ exists.\n");
      exit(1);
    }
    ...
    const tc_candle_info cnd_all = {
        .name = "all", .full_name = "All Candle Patterns", .pattern = TC_ALL};
